...
# - ストリーム表示制御: char-punct/batch/punctなど読みやすい表示モード
# - 外部設定: chat_config.json による表示・挙動設定の外部化
# - ストリーム機能のモジュール化: util/stream_render.py へ共通化
# - Python 3.13 対応: 型アノテーション強化とパフォーマンス最適化
...
from util.stream_render import (
    STREAM_PUNCT_CHARS_DEFAULT,
    invoke_with_optional_stream,
    get_post_response_wait_seconds,
    render_status_text,
)
...
def print_version_info():
    ...
    print("  🌊 ストリーム表示制御 - char-punct/batch/punct などに対応")
    print("  ⚙️ chat_config.json - 表示設定と挙動設定の外部化")
    print("=" * 60)
...
def _is_option_provided(option_names):
    """指定したオプションがCLIで明示指定されているかを判定する。"""
    return any(opt in sys.argv for opt in option_names)

def load_chat_config(config_path, config_explicit=False):
    """チャット設定JSONを読み込む。存在しない場合は空辞書を返す。"""
    if not config_path:
        return {}
    ...
    try:
        ...
        if not isinstance(data, dict):
            print(f"[WARNING] 設定ファイルの形式が不正です（オブジェクトを想定）: {config_path}")
            return {}
        return data
    except Exception as e:
        print(f"[WARNING] 設定ファイルの読み込みに失敗しました: {config_path} ({e})")
        return {}

def apply_chat_config_to_args(args, config):
    """設定ファイルの値を、CLI未指定の引数にのみ反映する。"""
    if not config:
        return

    option_flags = {
        'system_message': ['-s', '--system-message'],
        ...
        'stream_render_mode': ['--stream-render-mode'],
        'stream_punct_pause_ms': ['--stream-punct-pause-ms'],
        'stream_punct_chars': ['--stream-punct-chars'],
        'stream_batch_chars': ['--stream-batch-chars'],
        'stream_batch_delay_ms': ['--stream-batch-delay-ms'],
        'post_response_wait_ms': ['--post-response-wait-ms']
    }

    # 将来拡張用に stream セクションも受け付ける
    merged_config = dict(config)
    stream_section = config.get('stream')
    if isinstance(stream_section, dict):
        for key, value in stream_section.items():
            merged_config[f'stream_{key}'] = value

    for attr, flags in option_flags.items():
        if attr in merged_config and not _is_option_provided(flags):
            setattr(args, attr, merged_config[attr])
...
特別コマンド:
  ...
  'もとい' / 'ちゃいちゃい' : 複数行モードで直前の行を削除
    ...
    parser.add_argument("--config", type=str, default=CHAT_CONFIG_FILE,
                        help=f"Path to chat config JSON file. Default: {CHAT_CONFIG_FILE}")
    # ストリーム出力
    ...
    parser.add_argument("--stream-char-by-char", action="store_true",
                        help="Render stream output one character at a time")
    parser.add_argument("--stream-char-delay-ms", type=float, default=0.0,
                        help="Delay in milliseconds for each rendered character in stream-char-by-char mode")
    parser.add_argument("--stream-render-mode", type=str, default="chunk",
                        choices=["chunk", "char", "char-punct", "batch", "punct"],
                        help="Stream render mode: chunk|char|char-punct|batch|punct")
    parser.add_argument("--stream-punct-pause-ms", type=float, default=220.0,
                        help="Extra pause in milliseconds after punctuation for char-punct mode")
    parser.add_argument("--stream-punct-chars", type=str, default=STREAM_PUNCT_CHARS_DEFAULT,
                        help="Characters treated as punctuation boundaries in char-punct/punct modes")
    parser.add_argument("--stream-batch-chars", type=int, default=12,
                        help="Character count per flush in batch/punct stream render modes")
    parser.add_argument("--stream-batch-delay-ms", type=float, default=50.0,
                        help="Delay in milliseconds after each flush in batch/punct stream render modes")
    parser.add_argument("--post-response-wait-ms", type=float, default=-1.0,
                        help="Wait before showing execution-time line. -1 uses auto (typically 2x punctuation pause in char-punct mode)")
    # バージョン表示
    parser.add_argument("--version", action="version", version=f"Chat v{VERSION}")

    args = parser.parse_args()

    config_explicit = _is_option_provided(['--config'])
    chat_config = load_chat_config(args.config, config_explicit=config_explicit)
    apply_chat_config_to_args(args, chat_config)
    args.config_loaded = bool(chat_config)

    # 使いやすさ向上: 短いコマンドで読みやすい日本語ストリーム設定を適用
    if args.stream_readable:
        args.stream = True

        # 明示指定されていない項目だけをプリセットで補完
        if "--stream-render-mode" not in sys.argv:
            args.stream_render_mode = "char-punct"
        if "--stream-char-delay-ms" not in sys.argv:
            args.stream_char_delay_ms = 25.0
        if "--stream-punct-pause-ms" not in sys.argv:
...
def save_summary_to_file(summary_content, ai_assistant, model_name, zenn_mode=False, log_filename=''):
...
def main():
    ...
    try:
        ...
        print("Config file:", f"{args.config} ({'loaded' if args.config_loaded else 'not loaded'})")
        print("Stream output:", "ON" if args.stream else "OFF")
        if args.stream:
            if args.stream_readable:
                print("Stream preset:", "readable")
            print("Stream render mode:", args.stream_render_mode)
            if args.stream_render_mode in ["char", "char-punct"] or args.stream_char_by_char:
                print("Stream char delay (ms):", args.stream_char_delay_ms)
            if args.stream_render_mode == "char-punct":
                print("Stream punct pause (ms):", args.stream_punct_pause_ms)
                print("Stream punct chars:", args.stream_punct_chars)
            if args.stream_render_mode in ["batch", "punct"]:
                print("Stream batch chars:", args.stream_batch_chars)
                print("Stream batch delay (ms):", args.stream_batch_delay_ms)
                print("Stream punct chars:", args.stream_punct_chars)
            if args.post_response_wait_ms >= 0:
                print("Post response wait (ms):", args.post_response_wait_ms)
            else:
                print("Post response wait (ms):", "AUTO")
        print("----------------")
...
上記の会話を簡潔にまとめてください："""
                    ...
                    for attempt in range(max_retries):
                        try:
                            response_message, summary_streamed = invoke_with_optional_stream(
                                conversation,
                                summary_prompt,
                                args.stream,
                                f"📋 会話まとめ（{ai_assistant}:{model_name}）:",
                                render_mode=args.stream_render_mode,
                                char_by_char=args.stream_char_by_char,
                                char_delay_ms=args.stream_char_delay_ms,
                                punct_pause_ms=args.stream_punct_pause_ms,
                                punct_chars=args.stream_punct_chars,
                                batch_chars=args.stream_batch_chars,
                                batch_delay_ms=args.stream_batch_delay_ms
                            )
                    ...
                    if ai_response_content:
                        ...
                        # まとめをファイルに保存
                        summary_filename = save_summary_to_file(
                            ai_response_content, ai_assistant, model_name,
                            zenn_mode=zenn_mode,
                            log_filename=conversation_log_filename or ''
                        )
                    ...
                    post_wait_seconds = get_post_response_wait_seconds(args)
                    if post_wait_seconds > 0:
                        time.sleep(post_wait_seconds)
                    
                    render_status_text(
                        f"実行時間: {run_time:.3f}秒",
                ...
                for attempt in range(max_retries):
                    try:
                        response_message, response_streamed = invoke_with_optional_stream(
                            conversation,
                            user_question,
                            args.stream,
                            f"アシスタント（{ai_assistant}:{model_name}）:",
                            render_mode=args.stream_render_mode,
                            char_by_char=args.stream_char_by_char,
                            char_delay_ms=args.stream_char_delay_ms,
                            punct_pause_ms=args.stream_punct_pause_ms,
