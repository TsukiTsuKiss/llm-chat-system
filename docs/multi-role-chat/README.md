...
- **チーム会議 (並列処理・インタラクティブ対応)**: モデレーター機能付き高速会議システム
  ...
  - 🆕 **v1.1.0**: 並列処理対応（高速化）
- **phases ワークフローエンジン**: 直列・並列・ループ・サブルーチン呼び出しをネスト構造で管理
  ...
  - 🆕 **v1.6.0**: `phases` スキーマ（serial / parallel / loop）によるネストワークフロー
  - 🆕 **v1.6.0**: `type: "call"` によるサブワークフロー呼び出し（再利用可能なサブルーチン）
  - 🆕 **v1.6.0**: ログの `## 🔀 フロー設計` に Mermaid subgraph 展開（call 先の中身を可視化）
...
### 概要
事前に定義されたステップに従って、複数のロールが順次・並列・ループなどの構造で作業を行う機能です。
`phases` スキーマによりネスト構造を表現でき、`type: "call"` で別ワークフローをサブルーチンとして呼び出せます。
    ...
    "phases": [
      {"type": "loop", "max_iterations": 3, "exit_condition": "全員賛成",
       "phases": [{"type": "serial", "steps": [
         {"role": "司会", "action": "最有力案に絞り賛否を問う"},
         {"role": "メンバーA", "action": "賛否を答える"}
       ]}]}
    ]
  }
}
...
| フェーズタイプ | 説明 |
|---|---|
| `serial` | 前の発言を引き継ぎ順番に実行 |
| `parallel` | 全ロールに同じ入力を同時並列実行 |
| `loop` | `exit_condition` に合致するまで繰り返す |
| `call` | 別ワークフローをサブルーチンとして呼び出す |

### 利用可能なワークフロー（レガシースキーマ）
...
- **並列実行**：`parallel` フェーズで複数ロールを同時実行（コスト削減・高速化）
- **ループ**：`loop` フェーズで条件達成まで繰り返し（投票・レビューなど）
- **サブルーチン**：`call` フェーズで汎用ワークフローを他から再利用
- **専門性活用**：各ロールの専門知識を活用
- **Mermaid 可視化**：ログに subgraph 展開付きフロー図を自動埋め込み
- **記録保持**：全ステップの結果を保存

### --mermaid オプション（フロー図の事前確認）
  ...
  "roles": [
    {
      ...
      "role_type": "member"  // または "moderator"
    }
  ],
  "workflows": {
    "ワークフロー名": {
