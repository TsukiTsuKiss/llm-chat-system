...
Chat.py v8.0.0は1対1のAI会話に特化した高機能チャットシステムです。包括的なAPI制限エラー対応、トークン節約機能、自動まとめ機能を備えています。

## 🌟 新機能 (v8.0.0)

- **🚨 API制限エラー自動対応**: 413/429/503/504エラーの自動検出・対応・リトライ
- **💾 トークン節約機能**: まとめエントリの履歴除外によるトークン消費削減
- **📋 改良されたまとめ機能**: 会話履歴のAI要約とMarkdown形式保存
- **🔧 自動履歴削減**: リクエストサイズ超過時の会話履歴自動削減
...
📋 会話まとめ（Claude:claude-opus-4-20250514）:
## 会話の要約
...（まとめ内容）...
...
システムが自動的にエラーを検出・対応するため、手動対応は不要です。
