...
**セッション単位で OFF** にしたい場合は Web トグル（**Phase 5**、8.3 節）または CLI `--no-user-context` を使う（付録D）。
...
5. `minutes/` の JSON Schema（`minutes.schema.json`）は Phase 5（議事録化）で追加する。
   Phase 1〜4 では不要
...
`user_context` / `user_context_rag` は Phase 5 以降で実装する（付録D）。Phase 1〜4 ではスキップする。
...
| 方式 | 概要 | 採用 |
|---|---|---|
| **マーカー検出（推奨）** | 例: `【ユーザー確認】` を含む step 全文で `await_text`（または専用イベント）を発火。workflow に `interrupt_on` マーカー文字列を宣言 | Phase 5 で検討 |
...
- Phase 4: 予定 human / `await_choice` のみ（割り込みは未実装）
- Phase 5: 上記マーカー割り込みをエンジン + parity 試験で追加検討（議事録・user_context と合わせて優先度調整）
...
| `user_context` | ユーザーコンテキストを読み込んだか（付録D。Phase 5 以降） |
...
1. 左ペイン: 組織選択 / ワークフロー選択（「直接送信：全ロール」を先頭に含む）/
   ...
   ※ **ユーザーコンテキスト ON/OFF トグルは Phase 5**（付録D 実装時）。Phase 4 では非表示
...
4. **Phase 4（Web UI）**: Web 版（チャットタブ + 設定編集タブ + セッションタブ）。
   ...
   **user_context UI は Phase 5**（付録D）
5. **Phase 5（運用機能）**: セッション再開（分岐保存）、議事録化（7.3 節）、Git 連携（7.6 節）、
   **ユーザーコンテキスト**（付録D）、**ユーザー割り込み**（6.7 節・マーカー方式）、旧版からの完全移行完了（9.2 節）
...
- ユーザーコンテキストの詳細は付録D（構想）。RAG 拡張は D.10（Phase 6）。実装時に追記承認 UI の形を確定する
...
| **セッション** | Web トグル（**Phase 5 UI**） / CLI `--no-user-context` | 今回だけ OFF |
...
```
1. セッション終了時（またはユーザー操作）に「下地更新案」を生成
...
|---|---|
| 1〜4 | **実装しない**（1.5 節） |
| **5** | `my_context.md` 読み込み + ON/OFF + 5.1 注入 + 設定 UI |
| 5 | 更新案生成・承認 UI、要約版（D.7〜D.8） |
| **6** | **RAG 拡張**（D.10）：corpus / index / 関連 chunk 注入 |
