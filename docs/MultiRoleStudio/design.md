...
作成日: 2026-06-25 / 最終更新: 2026-07-15（Phase 5d-a・再開 UX 追補）

本書は MultiRoleStudio の仕様と設計判断をまとめたもの。
確定した仕様（第2〜10章）と、構想段階のアイデア（付録A〜C、付録D の 5d-b 以降）を明確に分けて記載する。
...
- **未実装（5d-b）**: 更新案生成・承認マージ（D.7）、要約版（D.8）、RAG（D.10 / Phase 6）
...
8. **レポート閲覧 UX**: 長い Markdown レポートは `max-height` + 内部スクロールの小窓に表示。操作ボタン（再開・議事録等）は枠外の下段に常時表示
...
4. **Phase 4（Web UI）**: Web 版（チャットタブ + 設定編集タブ + セッションタブ）。
   ...
   **user_context コア注入は Phase 5d-a**（付録D。更新案 UI は 5d-b）
5. **Phase 5（運用機能）**: セッション再開（分岐保存・**5a 実装済み**）、議事録化（7.3 節・**5b 実装済み**）、
   Git 連携・成果物採用（7.6 節・**5c 実装済み**）、**ユーザーコンテキスト コア注入（5d-a 実装済み・付録D）**、
   **ユーザー割り込み**（6.7 節・マーカー方式）、`workflow_bindings` フォーム（§8.4 残）、
   user_context 更新案・承認マージ（5d-b）、旧版からの完全移行完了（9.2 節）
...
## 付録D. ユーザーコンテキスト（Phase 5d-a 実装済み / 5d-b・RAG 未実装）
...
| **5d-b** | 更新案生成・承認 UI、要約版（D.7〜D.8） | 未実装 |
