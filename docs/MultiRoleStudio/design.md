...
会議とエンタメを別システムとして分けず、同じ実行基盤のモード差分として扱う（詳細は 7.4 節）。
MultiRoleChat でハードコードされていた会議モード・クイズモードは、ワークフローテンプレート（データ）に置き換える。

### 1.3 用語定義

演劇のアナロジーが構造にそのまま対応する。ファイル名 `MultiRoleStudio` もこれに由来する。
...
sessions/               ← 実行ログ（7章。JSONL のみ保存。Markdown は閲覧時に生成）
...
```json
{
  "session_id": "20260707_nokuru_meeting",
  "summary_version": 1,
...
3. ゲームモード: `ending_policy` と `state` 更新規則を重視（付録 A）
...
4. 「議事録」ボタン: 構造化サマリの生成とエクスポート（7.3 節）
...
1. **Phase 1 (MVP)**: `studio/` パッケージ + CLI 版。新データ構造（3章）、ワークフロー実行
   （serial/parallel/loop）、JSONL ログ、APIエラーリトライ、サンプルデータ（10章）。
   **着手順**: JSON Schema（3.7 節）とパリティ試験（9.3 節）を先に作成してから実装に入る
2. **Phase 2 (Web UI)**: Web 版（チャットタブ + 設定編集タブ + セッションタブ閲覧）
3. **Phase 3 (運用機能)**: セッション再開（分岐保存）、議事録化、旧版からの完全移行完了（9.2 節）

業務適用拡張フェーズ：

4. **Phase 4 (生成連携)**: 会話出力から TTS / ナレーション原稿 / ショット指示を生成
5. **Phase 5 (考査支援)**: 映像・音声・字幕の自動コンプラチェック用ルール評価を追加
6. **Phase 6 (運用基盤)**: 評価指標（品質・遅延・コスト）と MLOps 向け監視を追加
7. **Phase 7 (連携拡張)**: 外部ベンダー連携を前提に API 契約と入出力スキーマを固定
...
Web UI の項目（ファイル添付・設定編集）は Phase 2 実装時に UI テストとして追加する。
...
Phase 1 実装時に以下のサンプル一式を新規作成する（旧データからの変換はしない）。
...
「編成差し替えを行う参照型シナリオ」の3パターンを網羅し、動作確認とドキュメントの実例を兼ねる。
...
## 付録C. 外部プラットフォームへの参加（未確定・Phase 7 の具体像）
...
- Phase 7（連携拡張）で API 契約を固定する際は、この参加型を想定した
  入出力スキーマにしておくと移行しやすい
