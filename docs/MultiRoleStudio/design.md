...
studio/
  ...
  vcs.py         ← Git 連携（成果物の採用とコミット。7.6 節。Phase 3）
...
実装は `studio/vcs.py`（git コマンドのラッパー）。導入は Phase 3（運用機能）とする。
...
6. 「採用」ボタン: sandbox の成果物を作業ツリーへ適用し、コミットを作成する（7.6 節。Phase 3）
...
3. **Phase 3 (運用機能)**: セッション再開（分岐保存）、議事録化、Git 連携（7.6 節）、
   旧版からの完全移行完了（9.2 節）
